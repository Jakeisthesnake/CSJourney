...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    *output = strtol(str, &endptr, 0);
    ...
    else if(*output > upper_bound || *output < lower_bound){
        printf("out of bounds output > upper: %ld\n", (*output > upper_bound));
        printf("out of bounds output: < lower %ld\n", (*output < lower_bound));
        printf("out of bounds output: %ld, upper: %ld, lower %ld\n", *output, upper_bound, lower_bound);
        return -1;
    }
    else{
        return 0;
...
int translate_reg(const char* str) {
    ...
    //else if (strcmp(str, "$k1") == 0)   return 26;
    //else if (strcmp(str, "$k2") == 0)   return 27;
