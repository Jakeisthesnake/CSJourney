...
hex_to_str:
	# YOUR CODE HERE
    #t0 is 48 aka ascii code for 0
    #t1 is each 1 byte int
    #t2 is ascii of each 1 byte int
    #t3...?
    #t4 is address to load into
    #t5 is counter
    #t6 ... ?
    #t7 ...?
	#t8 is 9, last in in decimal
	#t9 is 97, ascii for 'a', aka 10 in decimal
	...
	sb $t1 8($a1)
	addiu $t4 $a1, 7
	addiu $t0, $zero, 48   #t0 is 48 aka 0 in ascii
	addiu $t5 $zero 8
	addiu $t8 $zero 9
	addiu $t9 $zero 97
hex_to_str_loop:	
	andi $t1, $a0, 0xF
	bgt $t1 $t8 hex_to_str_hex
	add $t2, $t1, $t0
	sb $t2, 0($t4)
	srl $a0 4
	addiu $t $t5 -1
	addiu $t4 $t4 -1
	bne $t5 $zero hex_to_str_loop 
	j hex_to_str_ret
hex_to_str_hex
    addiu $t1 $t1 -10
    add $t2, $t1, $t9
