...
addr_for_symbol:
	...
	sw $s0, 4($sp)
	sw $s1, 0($sp)
	add $s0, $a0, $zero                       # $s0 is pointer to next symbollist
addr_for_symbol_loop: 
	lw $s1, 0($s0)                           # look at what is in $s0
	beq $s1, $zero, addr_for_symbol_fail      # if $s0 is null go to fail
	lw $a0, 4($s0)                           # not null, so load lable string pointer into a1
	jal streq                               # check if labels are same ($a1 has query labe , $a0 has label of current node)
	beq $v0, $zero, addr_for_symbol_success   # if streq returns non zero, success
	addiu $s0, $s0, 8                       # add address of pointer to next node into $s0
	j addr_for_symbol_loop                  # representation
addr_for_symbol_success:
    add $v0, $s0, $zero
	lw $a1, 8($sp)
	lw $s0, 4($sp)
	lw $s1, 0($sp)
...
add_to_list:	
	# YOUR CODE HERE
	#s0 pointer to old node
	#s1 int address of new symbol
	#s2 pointer to begining of new symbol label
	#s3 pointer to new node
...
#make room for new stuff
	...
	addiu $a0, 12
	syscall
#error handle sys call
    addiu $t0, $zero, -1
	beq $v0, $t0, add_to_list_fail
	add $s3, $v0, $zero

#load new stuff to old location
new_list:
#make copy of lable string
    add $a0, $s2, $zero
	jal copy_of_str
	addiu $t0, $zero, -1
	beq $v0, $t0, add_to_list_fail
