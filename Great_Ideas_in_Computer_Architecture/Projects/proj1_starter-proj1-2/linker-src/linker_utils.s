...
inst_needs_relocation:
	...
	# t0 is funct of a0
	# t1 is op code of a0
	# t2 is op code for zero
	# t5 is op code for j
	# t6 is op code for jal
	
	and $t1 $a0 0xfc000000
	srl $t1 $t1 26
	addiu $t5 $zero 0x00000002
	beq $t1 $t5 inst_needs_relocation_yes
	addiu $t6 $zero 0x00000003
	beq $t1 $t6 inst_needs_relocation_yes
	add $v0 $zero $zero
	jr $ra
