...
if len(sys.argv) < 3:
    print('Usage: python compare_layers.py <file> <reference>')  # Change here
    sys.exit(2)
...
for i in range(layers):
    ...
    if invals[0] != 'LAYER' + str(i):
        print('ERROR: Invalid input data for layer ' + str(i))  # Change here
        sys.exit(2)

    refvals = refdata[i].split(',')
    if refvals[0] != 'LAYER' + str(i):
        print('ERROR: Invalid reference data for layer ' + str(i))  # Change here
        sys.exit(2)

    if len(refvals) != len(invals):
        print('ERROR: Dimensionality error in layer %d (expected: %d, was: %d)' % \
            (i, len(refvals), len(invals)))  # Change here
        sys.exit(2)
    ...
    for j in range(1, len(invals)):
        if abs(float(invals[j])-float(refvals[j])) > delta:
            print('ERROR: Value %d at layer %d is wrong: %f (should be %f)' % \
                (j, i, float(invals[j]), float(refvals[j])))  # Change here
            sys.exit(1)

print('OK')  # Change here